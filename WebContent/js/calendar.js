...
$(document).ready(function() {
    ...
    $('#calendar').fullCalendar({
        ...
        eventClick: function(calEvent, jsEvent, view) {
            ...
            $(".text_delete").on("click",function () {
            	$("#calendar").fullCalendar("removeEvents",evt_obj.id); //删除对应的事件,指定ID,没指定时删除所有事件  
            	setTimeout(setpopover,100);
            	//$('#calendar').fullCalendar('removeEventSource',evt_obj.id);
            });
        },
        editable: true,
        droppable: true,
        drop: function(date, allDay) {
    		...
    		copiedEventObject.id = myDate.getTime();
